...
setNewLoad(true)
    ...
    try {
      const response = await apiResponse.post("template/createtemplatecategory", {
        category: inputValue,
      });
  ...
  let DeleteCategory = async (id:string) => {
    ...
    let response = await apiResponse.delete(
      `template/deletetemplatecategory/${id}`
    );
  ...
  let fetchNewCat = async () => {
    setLoading(true)
    try {
      let response = await apiResponse.get("template/gettemplatecategory");
      if (response.status === 200) {
        let data = response.data?.data;
        Setcat(data); // Directly replace the state with the fetched data on initial render
      }
          ...
          {loading ? (
            ...
            <>
              {cat.map((value) => {
                return (
                  <div
                    className="inline-flex h-fit items-center justify-center rounded-full border border-purple-500 px-3 py-2 text-purple-700"
                    key={value?._id}
                  >
                    ...
                    <Dialog>
                      ...
                      <DialogContent className="sm:max-w-[425px]">
                        <div className="p-6 pt-0 text-center">
                          ...
                          <DialogTrigger asChild>
                            <Button className="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-base inline-flex items-center px-3 py-2.5 text-center mr-2 cursor-pointer" onClick={()=>{
